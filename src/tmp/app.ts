...
class Pad {
  ...
  private _drawStart(p: IPointData) {
    ...
    this.state.histroyIndex = Math.min(
      this.dynamicLayer.children.length - 1,
      this.state.histroyLength - 1
    );
  }
  ...
  private _seekHistroy(backIndex: number) {
    ...
    if (i < this.dynamicLayer.children.length - 1) {
      while (i < this.dynamicLayer.children.length - 1) {
        let o: Graphics = this.dynamicLayer.removeChildAt(
          this.dynamicLayer.children.length - 1
        ) as Graphics;
      ...
      while (
        i >= this.dynamicLayer.children.length &&
        this.tempGraphicsList.length > 0
      ) {
  ...
  private _signalRouterAction = (data: SignalData) => {
    if (data.type == SignalType.DRAG_EVENT) {
      ...
      if (
        d.hasOwnProperty("histroyBackIndex") &&
  ...
  private _mergeStaticLayer() {
    if (
      this.dynamicLayer == undefined ||
      this.dynamicLayer.children.length <= this.state.histroyLength
    )
      return;
    ...
    let texture: PIXI.Texture = this.app.renderer.generateTexture(
      this.staticLayer,
      { region: new PIXI.Rectangle(0, 0, padWH.w, padWH.h) }
    );
  ...
  public undo() {
    ...
    if (curr >= 0) {
      this.signalService.send(DataCenter.createHistroyIndexData(curr - 1));
    }
  }
  ...
  public clearAll() {
    ...
    this.signalService.send(DataCenter.createActionData(PadAction.CLEAR_ALL));
  }
  ...
  public clearAllData() {
    ...
    this.gpool.clear();
    this.signalService.send(
      DataCenter.createBrushTypeData(this.brushManager.lineStyle)
    );
  }
}

const padMode: PadMode =
  Utils.getQueryString("mode") == "reciver" ? PadMode.RECIVER : PadMode.SERVICE;
